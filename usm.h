...
namespace USMLIBRARY
{
	...
	public:
		...
		void End()
		{
			...
			hEventWrote = 0;
			if (hEventAux1)
				CloseHandle(hEventAux1);
			hEventAux1 = 0;
			if (hEventAux2)
				CloseHandle(hEventAux2);
			hEventAux2 = 0;
			if (hFM)
				CloseHandle(hFM);
			hFM = 0;
		...
		void CreateInit(const wchar_t* string_id, bool Init = false, unsigned long long csz = 1048576, DWORD MaxTh = 100, bool LocalOnly = true, bool Exex = false)
		{
			...
			if (LocalOnly)
			{
				...
				evwn = xup;
				swprintf_s(xup, 1000, L"%s_evaux1", stringid.c_str());
				evaux1 = xup;
				swprintf_s(xup, 1000, L"%s_evaux2", stringid.c_str());
				evaux2 = xup;
			...
			else
			{
				...
				evwn = xup;
				swprintf_s(xup, 1000, L"Global\\%s_evaux1", stringid.c_str());
				evaux1 = xup;
				swprintf_s(xup, 1000, L"Global\\%s_evaux2", stringid.c_str());
				evaux2 = xup;
		...
		int Initialize()
		{
			...
			if (hEventAux1 == 0)
				hEventAux1 = CreateEvAux1();
			if (hEventAux2 == 0)
				hEventAux2 = CreateEvAux2();
			if (hEventWrote == 0)
				return -1;
			if (Buff == 0)
		...
		DWORD NotifyWrite(bool Wait,DWORD how = INFINITE)
		{
			// Wait for all these threads to terminate reading
			return WaitForSingleObject(hEventWrote, Wait ? how : 0);
		}
